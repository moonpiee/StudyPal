...
session_id_runn="runn_chat_history7"
session_id_wo_runn="wo_runn_chat_history7"
st.title("Study Pal🌱📚💪")
...
instruction = instructions_lis[6]
...
match instruction:
    ...
    case "Tell Me Why":
          instruction_desc=f"Talk in {sel_language}.You like to explain WHY part of a wide range of topics, from the solar system, birds, and inventions to oceans, ancient worlds, cricket, animals, plants, transportation, and more. Tell one interesting fact or topic and explain the why part of it. One at a time. Pick topics that park curiosity and enhance learnining for children and adults. Ask no questions and no headers. Use emojis, symbols wherever if needed. Use markdown format"
    case "Tongue Twister":
...
if session_id_runn not in st.session_state and session_id_wo_runn not in st.session_state: #runnbale vs without runnable
    ...
    with st.spinner("Initializing the bot..."):
            ...
            # with st.chat_message("assistant"):
            #     st.markdown(initial_msg.content)
